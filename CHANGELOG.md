...
**Telegram-прокси теперь работает под VPN в любом клиенте — даже если тот «забыл» внести исключения: петля через туннель чинится на самой панели.**

- **🧲 Прокси больше не теряется внутри собственного VPN.** Если клиент заворачивал соединение к прокси в туннель (так делает INCY без применённого профиля), серверный Xray по сниффину SNI-маскировки уводил его на адрес фронта (www.microsoft.com) в порт 7443 — там никто не слушает: вечные SYN-штормы, а прокси «не видит подключений». Фронт-домены теперь исключены из подмены назначения (`domainsExcluded`), и петля «туннель → свой же :7443» честно доходит до telemt. Работает без каких-либо ручных исключений в клиенте — в Happ, INCY и любом Xray-клиенте.
- **🛡 Защита порта прокси (veil_synfix) доработана:** собственные адреса сервера (IPv4 и IPv6) выведены из лимита SYN — туннельная петля не упирается в «54 в минуту»; для IPv6-петли появились отдельные счётчики.
- **📡 MTProto dual-stack:** telemt поднимает слушатели и на IPv4, и на IPv6 (`[network] ipv6 = true`), iOS с IPv6-маршрутом под VPN больше не теряет прокси. В конфигах WireGuard/AmneziaWG из туннеля исключаются оба адреса сервера.
- **📲 INCY:** профиль маршрутизации приезжает диплинк-формой `incy://routing/onadd/…` (без схемы ядро молча его игнорировало), ссылки Telegram-прокси (`tg://`) добавлены и в INCY-подписку; WG/AmneziaWG-эндпоинт в INCY-ссылках — IPv4-литерал (пинг Incy не резолвит домены).
- **🔒 SNI-фронт:** живая проверка X25519MLKEM768 у фронта с бейджем в интерфейсе; дефолт install.sh переключён на `www.microsoft.com` (фронт `my.aeza.ru` отдаёт только TLS 1.2, iOS капризничает).
- **🧰 Синтакс-защита MTProto-порта** (по мотивам MEKO V3): iOS-клиенты узнаются по отпечатку SYN и не лимитируются, остальным — 54 SYN/мин, сверх — отбой; пересборка правил при старте/смене порта/дрейфе, тумблер с простым объяснением в «Настройках».
- **🌐 Веб-прокси:** транспорт по умолчанию — WebSocket (стабильнее всего на практике).
- **📄 Мелочи:** активная вкладка возвращается после перезагрузки страницы (мобильные браузеры выгружают фоновые вкладки).
