...
- **Заглушка на 443 переписана целиком** («8BIT HAVEN»). Старая страница была нерабочей: в её скрипте была синтаксическая ошибка (`pixel & 0FF00` — невалидный литерал, из-за чего умирал весь `<script>`), демо-ром был `nestest.nes` (тестовый дамп без графики — экран оставался чёрным), а аудиобуфер и ввод не работали.
- **Рабочая эмуляция**: корректный blit покадрового буфера JSNES в `Uint32Array` (с учётом порядка байт), плавающий аудиобуфер через `ScriptProcessor` без двойной нормализации семплов, пауза/звук/CRT-фильтр/полноэкранный режим, чёрный экран-заглушка до старта.
- **Легальные домашние ромы вместо nestest**: `roborun.nes` (RoboRun 1.1, GPL-3.0, автор heaven-hm) и `falling.nes` (Falling, MIT, автор tragicmuffin) — обе NROM, 40976 B, запускаются на JSNES. В `decoy/` добавлены `ROMS.md` (происхождение, лицензия, предложение исходников по GPL-3.0 §6) и тексты лицензий `LICENSE-ROBORUN` / `LICENSE-FALLING`; `nestest.nes` удалён.
- **Управление**: мультитач-геймпад на pointer-событиях (пер-элементный `held`-сет + `setPointerCapture`), клавиатура (стрелки/WASD, Z/X, Enter/Shift, P/M/C/F), Gamepad API с опросом в кадровом цикле, вибро-отклик, загрузка **своего** `.nes` локально в браузере (файл никуда не отправляется) с проверкой заголовка iNES `4E 45 53 1A`.
- **Побочный эффект для заглушки как cover**: любой запрос (включая произвольные пути) по-прежнему отдаёт содержательный HTML вместо «not found» — страница выглядит как реальный игровой сайт, а не как пустая заглушка.
- **Operational**: `telemt` кэширует каталог `decoy` при старте и не подхватывает добавленные файлы, поэтому `install.sh` теперь перезапускает `telemt` после переустановки с сохранением данных (`systemctl is-active --quiet telemt && systemctl restart telemt`).
